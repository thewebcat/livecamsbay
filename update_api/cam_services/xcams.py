...
class XcamsShowApi(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context = super(XcamsShowApi, self).get_context_data(**kwargs)
        gender = []
...
class XcamsUpdateAllApi(View):
    ...
    def get(self, *args, **kwargs):
        ...
        for i in cam_data.findall('./webcam'):
            ...
            data = {
                'name': cam.prefix + i.attrib['account'],
                'display_name': i.find('./nickname').text,
                'profile_image': i.find('./webcam_previews/preview[@width="260"]').text,
                'age': i.find('./age').text,
                'available': False,
                'cam_service': cam,
                'chat_url': 'https://secure.dnxlive.com/en/login/'+i.find('./nickname').text+'/',
...
class XcamsUpdateApi(View):
    ...
    def get(self, *args, **kwargs):
        #cam, data = load_api_data('jasmin')
        cam = CamService.objects.get(prefix='xcams')
        response = urllib.urlopen("https://cams.dnxlive.com/xml/v0.3/onlineCamsStatus.xml")
        cam_data = ET.parse(response)
        ...
        for i in cam_data.findall('./webcam'):
            try:
                check_online = Model.objects.get(name=cam.prefix + i.attrib['account'])
        ...
        Model.objects.filter(cam_service__prefix='xcams').exclude(name__in=changed_objects).update(available=False,
