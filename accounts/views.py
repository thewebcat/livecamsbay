...
from accounts.models import Profile, ViewInstance
...
from common.decorators import render_to, render_to_json, set_views, has_edit
from accounts.forms import OrderAddOrEditForm, AddOfferToOrderForm, PasswordChangeForm
...
@render_to("accounts/note_list.html")
@validate_access()
def note_list(request):
    ...
    """
    return {
        'notes': to_pagination(Note.objects.filter(profile=request.user.profile), page=request.GET.get('page'))
    }


@render_to("accounts/note_add_or_edit.html")
@validate_access()
def note_add(request):
    """
    Добавление заметки
    """
    return note_add_or_edit(request, note_id=None)


@render_to("accounts/note_add_or_edit.html")
@validate_access()
def note_edit(request, object_id=None):
    """
    Редактирование заметки
    """
    output = note_add_or_edit(request, note_id=object_id)

    return output


def note_add_or_edit(request, note_id=None):
    """
    ...
    initial = {}
    if note_id:
        note = get_object_or_404(Note, id=note_id, profile=request.user.profile)
    else:
        import datetime
        note = Note()
        initial = {'notify_date': datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')}
    ...
    form = NoteForm(instance=note, initial=initial)
    return {'form': form}
